...
Blockly.Arduino['unipolar_stepper_motor_init'] = function(block) { //Attention l'usine à bloc ajoute Blockly.Arduino['unipolar_stepper_motor_init'] au lieu de Blockly.Arduino.unipolar_stepper_motor_init
  ...
  Blockly.Arduino.includes_['stepper'] = '#include <Stepper.h>\n' ; //bibliothèques. À quoi sert ce qui est entre crochet ? C'est pour éviter que soit inclue 2 fois la même bibliothèque, en effet, si un autre bloc utilise la même librairie et que ce qui a entre crochet est similaire, ardublock n'ajoutera la librairie une seule fois.
//  Blockly.Arduino.variables_[] = 'placer ici le code de mes variables' ;//placer ici le code de mes variables
  Blockly.Arduino.definitions_['declarationStepperMotor'] = 'Stepper '  + variable_unipolar_stepper_motor_variable + '(' + value_unipolar_stepper_motor_number_of_step_by_revolution + ', ' + value_unipolar_stepper_motor_in2 + ', ' + value_unipolar_stepper_motor_in4 + ', ' + value_unipolar_stepper_motor_in3 + ', ' + value_unipolar_stepper_motor_in1 + ');\n'; //placer ici le code de mes instances. Astuces 1) Pour le nom des variables, prendre ce qui est à gauche ci-dessus var xxxxx 2) si on met + xxx + pour mettre les variables, il faut qu'il y ait du code orange à droite aussi (ex. ');' sinon ça ne marche pas)
