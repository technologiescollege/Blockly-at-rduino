...
Examples.initLocal = function() {
    ...
    success: function(data) {
		for (var i = 0; i < data.length; i++) {
			...
			current = $(current).next('td');
			if (data[i].visible) {
				$(current).find('.checkbox-inline').append('<input data-toggle="toggle" type="checkbox" data-onstyle="success" checked="checked">');
			} else {
				$(current).find('.checkbox-inline').append('<input data-toggle="toggle" data-onstyle="success" type="checkbox">');
			}
			$('table').append(clone);
		}
        $('input[type=checkbox][data-toggle^=toggle]').bootstrapToggle();
...
Examples.initServer = function() {
	...
	$("#addDir").on("click",	function(e) {
		...
		if ( $("#directory option[value='"+newDir+"']").length == 0 ) {
			$("#directory").append('<option value="'+newDir+'" selected="selected">'+newDir+'</option>');
			$('#divNewDir').addClass("hide");
	        if ($('#inputFile').val()) {
	            $('#btn_submit').removeAttr("disabled");
	        }
	...
	$("#generate").on("click",	function() {
		...
		$.ajax({
			url: "generate.php",
			type: "POST",
			data:  {'examples': JSON.stringify(lignes, null, 4)},
			success: function(data){
    ...
    function beforeSubmit(){
    	      ...
    	      switch(ftype)  {
    	            ...
    	            break;
    	            default:
    	            $("#msgUpload").html("<b>"+ftype+"</b> "+Examples.Msg.fileType+"");
    	            $("#msgUpload").removeClass();
    	            $("#msgUpload").addClass("alert alert-danger");
    	         	return false;
    	       }
...
// Populate the language selection menu.
  ...
  $('#directory').children('option:first').text(Examples.Msg.Html.new_directory);
  $( ".glyphicon-upload" ).after("<span>"+Examples.Msg.Html.span_addfile+"</span>");
};
